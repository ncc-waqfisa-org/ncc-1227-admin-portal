...
import { MasterLog } from "../../../../src/API";
import { useRouter } from "next/router";
import { getMasterStudentLogsByLogID } from "../../../../src/CustomAPI";
import { MasterApplicationSnapshot } from "../../../../src/Helpers";
import { PageComponent } from "../../../../components/page-component";
import PrimaryButton from "../../../../components/primary-button";
import { Toaster } from "react-hot-toast";

interface Props {
  studentLog: MasterLog;
}
export const getServerSideProps: GetServerSideProps = async (ctx) => {
  const { id } = ctx.query;
  const { locale } = ctx;

  let studentLog = await getMasterStudentLogsByLogID(`${id}`);
  return {
    props: {
...
const StudentLogHistoryInfo = ({ studentLog }: Props) => {
  ...
  const [snapshot, setSnapshot] = useState<
    MasterApplicationSnapshot | undefined
  >(undefined);

  useEffect(() => {
    if (studentLog.snapshot) {
      setSnapshot(JSON.parse(studentLog.snapshot) as MasterApplicationSnapshot);
    }
        ...
        <div className="p-4 ">
          ...
          <div className="flex flex-col justify-between gap-4 ">
            <div>
              ...
              <div>{`${studentLog.student?.m_firstName} ${studentLog.student?.m_secondName} ${studentLog.student?.m_thirdName} ${studentLog.student?.m_lastName}`}</div>
