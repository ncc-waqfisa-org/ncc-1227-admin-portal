...
type TScholarshipForm = {
  scholarship: Scholarship;
};

export const ScholarshipForm: FC<TScholarshipForm> = ({ scholarship }) => {
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      IBAN: scholarship.IBAN ?? undefined,
      bankName: scholarship.bankName ?? undefined,
      isConfirmed: scholarship.isConfirmed ?? false,
  ...
  async function uploadAndUpdate(values: z.infer<typeof formSchema>) {
    ...
    const unsignedContractKey = values.unsignedContractFile
      ? uploadFile(
          ...
          scholarship.studentCPR ?? "undefined"
        )
      : Promise.resolve(scholarship.unsignedContractDoc);
    ...
    const signedContractKey = values.signedContractFile
      ...
      : Promise.resolve(scholarship.signedContractDoc);
    ...
    const IBANLetterKey = values.IBANLetterFile
      ...
      : Promise.resolve(scholarship.IBANLetterDoc);
    ...
    let updateVariables: UpdateScholarshipMutationVariables = {
      input: {
    ...
    return updateSingleScholarship(updateVariables);
  }
    ...
    await toast
      ...
      .then(async (value) => {
        ...
        queryClient.invalidateQueries({
          queryKey: ["scholarship", scholarship.id],
        });
        push("/scholarships");
      });
  }
              ...
              <FormItem className="sm:col-span-2">
                <FormControl>
                  <MultiUpload
                    single
                    required={false}
                    storageKeys={[scholarship.unsignedContractDoc ?? null]}
                    onFiles={(files) => {
              ...
              <FormItem className="">
                <FormControl>
                  <MultiUpload
                    single
                    required={false}
                    storageKeys={[scholarship.IBANLetterDoc ?? null]}
                    onFiles={(files) => {
                      field.onChange(files.length > 0 ? files[0] : undefined);
                    }}
                    isInvalid={(isInvalid) =>
              ...
              <FormItem className="">
                <FormControl>
                  <MultiUpload
                    single
                    required={false}
                    storageKeys={[scholarship.signedContractDoc ?? null]}
                    onFiles={(files) => {
                      field.onChange(files.length > 0 ? files[0] : undefined);
                    }}
                    isInvalid={(isInvalid) =>
