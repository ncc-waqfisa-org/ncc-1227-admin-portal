...
export default function MasterBatchUpdateForm(props) {
  const {
    batch: batchProp,
    masterBatch: masterBatchModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = masterBatchRecord
      ? { ...initialValues, ...masterBatchRecord }
      : initialValues;
  ...
  const [masterBatchRecord, setMasterBatchRecord] =
    React.useState(masterBatchModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = batchProp
        ? await DataStore.query(MasterBatch, batchProp)
        : masterBatchModelProp;
      setMasterBatchRecord(record);
    };
    queryData();
  }, [batchProp, masterBatchModelProp]);
  React.useEffect(resetStateValues, [masterBatchRecord]);
  const validations = {
    batch: [{ type: "Required" }],
    createApplicationStartDate: [],
    createApplicationEndDate: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            MasterBatch.copyOf(masterBatchRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MasterBatchUpdateForm")}
      {...rest}
    >
      <TextField
        label="Batch"
      ...
      >
        <Button
          ...
          isDisabled={!(batchProp || masterBatchModelProp)}
