...
export default function StatisticsUpdateForm(props) {
  const {
    id: idProp,
    statistics: statisticsModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = statisticsRecord
      ? { ...initialValues, ...statisticsRecord }
      : initialValues;
  ...
  const [statisticsRecord, setStatisticsRecord] =
    React.useState(statisticsModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(Statistics, idProp)
        : statisticsModelProp;
      setStatisticsRecord(record);
    };
    queryData();
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            Statistics.copyOf(statisticsRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "StatisticsUpdateForm")}
      {...rest}
    >
      <TextField
        label="Id"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || statisticsModelProp)}
