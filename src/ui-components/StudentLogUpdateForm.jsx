...
export default function StudentLogUpdateForm(props) {
  const {
    id,
    studentLog,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...studentLogRecord };
    setApplicationID(cleanValues.applicationID);
    setStudentCPR(cleanValues.studentCPR);
    setDateTime(cleanValues.dateTime);
    setSnapshot(cleanValues.snapshot);
    setReason(cleanValues.reason);
    setErrors({});
  };
  const [studentLogRecord, setStudentLogRecord] = React.useState(studentLog);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(StudentLog, id) : studentLog;
      setStudentLogRecord(record);
    };
    queryData();
  }, [id, studentLog]);
  React.useEffect(resetStateValues, [studentLogRecord]);
  const validations = {
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            StudentLog.copyOf(studentLogRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "StudentLogUpdateForm")}
    >
      <TextField
